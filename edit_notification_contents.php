...
use local_nudge\form\nudge_notification_content\edit;
...
\admin_externalpage_setup('configurenudgenotificationcontents');
...
$base_edit_url = new moodle_url('/local/nudge/edit_notification_contents.php', [
    'sesskey' => \sesskey()
]);

$manageurl = new \moodle_url('/local/nudge/edit_notification_contents.php');
...
if ($delete) {
    ...
    $notification_to_delete = nudge_notification_content_db::get_by_id($delete);
    ...
    if (!$confirm) {
        ...
        echo $OUTPUT->confirm(
            get_string(
                'deletenudgenotificationcontentconfirm',
                'local_nudge',
                format_string($notification_to_delete->subject)
            ),
            $confirm_url->out(),
            $cancel_url
        );
    ...
    nudge_notification_content_db::delete($delete);
...
if ($edit) {
    ...
    $nudge_notification = nudge_notification_content_db::get_by_id($edit);
    ...
    if ($mform->is_cancelled()) {
        ...
        nudge_notification_content_db::save($nudge_notification);
        \redirect($base_edit_url);
    }
}
...
if ($add) {
    ...
    $new_notification = new nudge_notification_content();
    $new_notification_id = nudge_notification_content_db::save($new_notification);
...
// We want to manage the notification contents, no actions have been taken.
echo $OUTPUT->header();

/// ========= BEGIN NUDGE_NOTIFICATION_CONTENT ========= ///
$add_new_url = clone $base_edit_url;
$add_new_url->param('add', 1);
echo $OUTPUT->single_button($add_new_url, 'Add notification content', 'get');

$table = new \flexible_table('Testing');
$table->define_baseurl(new \moodle_url('/local/nudge/edit_notification_contents.php'));
$table->define_columns(['lang', 'subject', 'body', 'actions']);
$table->define_headers(['Language', 'Subject', 'Body', 'Actions']);
...
$select_nudgenotifications_sql = <<<SQL
    SELECT
        *
    FROM
        {nudge_notification_content}
SQL;

/**
 * @var array<int, nudge_notification_content>
 */
$notifications = $DB->get_records_sql($select_nudgenotifications_sql);

foreach ($notifications as $notification) {

    $body_trimmed = \trim(\substr($notification->body, 0, 20));
    $language_options = \get_string_manager()->get_list_of_languages();
    $row_fields = [
        (isset($language_options[$notification->lang])) ? $language_options[$notification->lang] : 'Unknown',
...
$table->finish_html();
/// ========= END NUDGE_NOTIFICATION_CONTENT ========= ///
